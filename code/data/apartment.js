...
if (!occupantCapacity) throw 'You must provide occupantCapacity of the apartment';
...
async getAllApartmentSelectedZipCode(zipcode){
  if (!zipcode) throw 'You must provide an id to search for';
    ...
    const apartmentCollection = await apartment();
    const apartmentData = await apartmentCollection.find({
      zipcode: zipcode
    }).toArray();
...
async getApartmentById(apartmentId){
  if (!apartmentId) throw 'You must provide an apartment id to search for';
    ...
    const apartmentCollection = await apartment();
    const apartmentIdData = await apartmentCollection.find({
...
function yearValidation(year) {
 ...
 if (year !== 0) {
 if ((year !== "") && (!text.test(year))) {throw "Please Enter Numeric Values Only"; }
 ...
 if(year<1900 || year > 2022) throw 'Only years 1900-2022 are the valid values'; 
  }
else {throw 'Year can not be 0'}; 
}
